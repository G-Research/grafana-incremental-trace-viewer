...
const HELP_CONFIG = {
  'panel-too-small': {
    ...
    color: 'text-gray-900 light:text-gray-900 dark:text-white',
...
const markdownComponents = {
  h1: ({ children, ...props }: any) => (
    <h1 className="text-2xl font-bold text-gray-900 light:text-gray-900 dark:text-white mb-4 mt-8 pt-8" {...props}>
      {children}
    </h1>
  ),
  h2: ({ children, ...props }: any) => (
    <h2 className="text-xl font-bold text-gray-900 light:text-gray-900 dark:text-white mb-3 mt-6 pt-6" {...props}>
      {children}
    </h2>
  ),
  h3: ({ children, ...props }: any) => (
    <h3 className="text-lg font-semibold text-gray-900 light:text-gray-900 dark:text-white mb-2 mt-4 pt-4" {...props}>
      {children}
    </h3>
  ),
  p: ({ children, ...props }: any) => (
    <p className="text-sm text-gray-700 light:text-gray-700 dark:text-gray-300 mb-2 mt-2 pt-2" {...props}>
      {children}
    </p>
  ),
  ul: ({ children, ...props }: any) => (
    <ul
      className="text-sm text-gray-700 light:text-gray-700 dark:text-gray-300 space-y-1 list-disc list-inside mb-3 mt-3"
      {...props}
    >
      {children}
    </ul>
  ...
  li: ({ children, ...props }: any) => (
    <li className="text-sm text-gray-700 light:text-gray-700 dark:text-gray-300" {...props}>
      {children}
    </li>
  ),
  a: ({ children, href, ...props }: any) => (
    <a
      href={href}
      className="text-blue-600 light:text-blue-600 dark:text-blue-500"
      target="_blank"
  ...
  code: ({ children, className, ...props }: any) => {
    ...
    if (isInline) {
      return (
        <code className="text-green-700 light:text-green-700 dark:text-green-400" {...props}>
          {children}
        </code>
      );
    }
    return (
      <div className="bg-gray-100 light:bg-gray-100 dark:bg-gray-800 rounded p-3 mb-3">
        <code className="text-green-700 light:text-green-700 dark:text-green-400 text-sm" {...props}>
          {children}
        </code>
      </div>
    );
  ...
  blockquote: ({ children, ...props }: any) => (
    <blockquote
      className="bg-blue-50 light:bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 pl-4 py-2 mb-3"
      {...props}
    >
      <p className="text-sm text-blue-700 light:text-blue-700 dark:text-blue-300">{children}</p>
    </blockquote>
  ),
      ...
      <div className="max-h-[70vh] overflow-y-auto">
        {isLoading ? (
          <div className="flex items-center justify-center p-8">
            <div className="text-gray-500 light:text-gray-500 dark:text-gray-400">Loading help content...</div>
          </div>
        ) : (
